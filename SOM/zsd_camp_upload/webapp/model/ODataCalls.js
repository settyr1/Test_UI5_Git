sap.ui.define([
		...
		return Object.extend("zsdcampupload.model.ODataCalls", {
			...
			constructor: function(oModel, oViewModel, oComponent, oResourceBundle) {
				...
				var sPath1 = jQuery.sap.getModulePath("zsdcampupload", "/model/fileAttributes.json");
				var oFileMdl = new JSONModel(sPath1).attachRequestCompleted({}, function() {
					this._oDataResults = this._oLocalJsonMdl.getData();
				}, this);
				this._oLocalJsonMdl = oFileMdl;
				
				var sPath = jQuery.sap.getModulePath("zsdcampupload", "/model/mandatoryFields.json");
				this.oMandatoryFields = new JSONModel(sPath).attachRequestCompleted({},function(oEvent1) { oEvent1.getSource().getData(); },null);
			...
			getFilePropertySlug: function () {
				return this._oDataResults.Campaign      + "|" + 
				       this._oDataResults.Occurance     + "|" + 
				       this._oDataResults.FileDelimiter + "|" + 
				       this._oDataResults.OccurenceDate ;                 
			},
...
// New Array to append to JSON Model	
		...
		DeleteNewBrand: function(oEvent){
						...
						oData.data.results.forEach(function(item, index, array){
							...
							if ( item.Outputstyle === '01' ) {  //output
								...
								var lFieldName = $(vEntityProperty).find(function(oField){
										if( oField.name && item.Fieldname.replace(/_/g, '') === oField.name.toUpperCase() ){
											return true;
										}
									}
							);	
			...
			readSearchHelpResults: function(oFilter, lSearchHelpName, lSearchService,EntityProperty,lfragUom){

				var myresponse;
				var myodata;
				var lEntitySetToCall;
				
				this.lfragUom = lfragUom;
				...
				oDataModelSearchHelpResult.read(lEntitySetToCall, {
					...
					success: function(response, oData) {
								...
								if( oData != null )
								{
									...
									if(this.lfragUom){
										this.lfragUom.forEach(function(item, index, array) {
...
// 				this.mandatoryBrandMaterialFields = this.oMandatoryFields.getData();
