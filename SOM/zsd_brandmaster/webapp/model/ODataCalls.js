sap.ui.define([
		...
		return Object.extend("zsdbrandmaster.model.ODataCalls", {
			...
			constructor: function(oModel, oViewModel, oComponent, oResourceBundle) {
				...
				this.oMandatoryBrands = new JSONModel(sPath).attachRequestCompleted({},function(oEvent1) { oEvent1.getSource().getData(); },null);
...
//set busy indicator
				...
				jQuery.when(this.oFetchMainDeferred.promise()
						...
						if(oMainPromisResult){
							this._oDataResults.BrandsSet = oMainPromisResult.results;
						}
			...
			//  this.showBusyIndicator(true);
				this.clearJsonModel();
// BrandsSet?$skip=0&$top=20&$orderby=Brand%20asc 
...
//get brand dropdown list				
			...
			saveDeepEntity: function(){
				...
				deepEntityData.BrandsSetDummy = requestData.BrandsSet;
//set busy indicator
				this.oGlobalBusyDialog.open();
				...
				this._oDataModel.create("/BrandsSet", deepEntityData, {
					success: function(oResponse) {
						...
						if( oResponse.BrandsSetDummy.results ) {
						this._oDataResults.BrandsSet = oResponse.BrandsSetDummy.results;
						} else {
							this._oDataResults.BrandsSet = {};
						}
		...
		onDataExportBrandsSet : function(oEvent) {
// https://help.sap.com/viewer/468a97775123488ab3345a0c48cadd8f/7.4.19/en-US/f1ee7a8b2102415bb0d34268046cd3ea.html
			...
			oExport.saveFile("BrandMaster").catch(function(oError) {
...
// New Array to append to JSON Model	
				...
				this._oDataResults.BrandsSet.push(obj);
				this.refreshUIScreen();
		},
		...
		DeleteNewBrand: function(oEvent){
			...
			if(selectedIndex){
				this._oDataResults.BrandsSet.splice(selectedIndex, 1);
				this.refreshUIScreen();
			}
		},
...
//validate table fields
				this.mandatoryBrandsFields = this.oMandatoryBrands.getData();
				if(this.mandatoryBrandsFields  && this._oDataResults.BrandsSet){
//loop at all rows
					$.each(this._oDataResults.BrandsSet, function(sytabix,ls_BrandsSet){
//loop at all fields in a row

						if( ls_BrandsSet.DelInd === true){
							return;
						}
						var emptyRow = "";
						//console.table(this._oDataResults.PgeneralToPPG04);
						$.each(this.mandatoryBrandsFields, function(manfname,fvalue){
							var errFname = manfname + "Err";
							ls_BrandsSet[errFname] = "None";
							if(ls_BrandsSet[manfname] === "" || parseFloat(ls_BrandsSet[manfname], 2) === 0 || ls_BrandsSet[manfname] !== ls_BrandsSet[manfname]){
								ls_BrandsSet[errFname] = "Error";
								this.validationError = true;
							} else {
								emptyRow = emptyRow + ls_BrandsSet[manfname];
							}
						}.bind(this));
