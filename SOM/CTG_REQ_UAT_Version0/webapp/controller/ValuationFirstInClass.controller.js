sap.ui.define([
	...
	return Controller.extend("com.pfizer.ctg.CTG_REQ.controller.ValuationFirstInClass", {
		...
		_onObjectMatched: function (oEvent) {
			if (oEvent.getParameters().name !== "ValuationFirstInClass") {
				return;
			}
			...
			aAttachments = [];
			this.attmntSaved = "";
			this.reqNo = oEvent.getParameters("ValuationFirstInClass").arguments.reqId;
			var pad = "0000000000";
			this.reqNo = (pad + this.reqNo).slice(-pad.length);
			this.priceModel = oEvent.getParameters("ValuationFirstInClass").arguments.priceModel;
			this.action = oEvent.getParameters("ValuationFirstInClass").arguments.action;
		...
		onIconTabSelect: function (oEvent) {
			...
			if (oEvent.getSource().getSelectedKey().indexOf("iTFValuPrdTyp") >= 0) {
				if (this.action === "UPD-VFC") {
					...
					//Setting Visible property for Approve Request Button to false when Send to Approver is Enabled as true by DOGIPA
					this.getView().byId("apprvReq").setVisible(false);
					...
					this.getView().byId("saveReq").setVisible(false);
					this.getView().byId("sendToApprv").setVisible(false);
					//Setting Visible property for Approve Request Button to true when Send to Approver is Enabled as false by DOGIPA
					if (reqHExtnModel.getData().ReqHExtnSet.HStatus === "AC") {
						this.getView().byId("apprvReq").setVisible(false);
					} else {
						this.getView().byId("apprvReq").setVisible(true);
					}
		...
		onSelectUploadFile: function (oEvent) {
			...
			reader.onload = function (event) {
				...
				content.ReqNo = content.ReqNo.pad(10, "0");
			...
			for (i = 0; i < oTable.getItems().length; i++) {
				if (oTable.getItems()[i].getSelected()) {
					...
					if (oFound.length === 0) {
						...
						sAttFile.ReqNo = sAttFile.ReqNo.pad(10, "0");
