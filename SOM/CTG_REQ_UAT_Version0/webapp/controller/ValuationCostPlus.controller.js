sap.ui.define([
	...
	return Controller.extend("com.pfizer.ctg.CTG_REQ.controller.ValuationCostPlus", {
		...
		_onObjectMatched: function (oEvent) {
			if (oEvent.getParameters().name !== "ValuationCostPlus") {
				return;
			}
			...
			aAttachments = [];
			this.attmntSaved = "";
			this.reqNo = oEvent.getParameters("ValuationCostPlus").arguments.reqId;
			var pad = "0000000000";
			this.reqNo = (pad + this.reqNo).slice(-pad.length);
			this.priceModel = oEvent.getParameters("ValuationCostPlus").arguments.priceModel;
			this.action = oEvent.getParameters("ValuationCostPlus").arguments.action;
		...
		updateViewElements: function () {
			...
			if (reqHExtnModel.getData().ReqHExtnSet.HStatus === "DR" ||
				...
				this.getView().byId("iTFValuCostPlus").setVisible(false);
			} else {
				this.getView().byId("iTFValuCostPlus").setVisible(true);
				var iTabTitle = "Valuation";
				iTabTitle = iTabTitle.concat("(", reqHExtnModel.getData().ReqHExtnSet.ProdTyp, ")");
				this.getView().byId("iTFValuCostPlus").setText(iTabTitle);
			}
			var oElement = " ";
			...
			var costPerUnitLbl = "Cost per Unit (USD/";
			costPerUnitLbl = costPerUnitLbl.concat(reqHExtnModel.getData().ReqHExtnSet.QtyUOM, ")");
			this.getView().byId("costPerUnitLbl").setText(costPerUnitLbl);
			var valuAddedLbl = "Value Added by Pfizer (USD/";
			valuAddedLbl = valuAddedLbl.concat(reqHExtnModel.getData().ReqHExtnSet.QtyUOM, ")");
			this.getView().byId("valuAddedLbl").setText(valuAddedLbl);
			...
			if (reqHExtnModel.getData().ReqHExtnSet.HStatus === "AC" || reqHExtnModel.getData().ReqHExtnSet.HStatus === "EX") {
				if (userInfoModel.getData().ReqstorSet.Role === "CREATOR" || userInfoModel.getData().VFCMgrSet.Role === "VALUATOR") {
					if (reqHExtnModel.getData().ReqHExtnSet.CurrReqNo) {
						this.getView().byId("actionsButn").setVisible(false);
					} else {
						var expDate = this.getView().getModel("reqHExtnModel").getData().ReqHExtnSet.ExpDateYMD;
						...
						if (currentDate >= expiryDate && currentDate <= futureDate) {
		...
		onIconTabSelect: function (oEvent) {
			...
			if (oEvent.getSource().getSelectedKey().indexOf("iTFValuCostPlus") >= 0) {
				if (this.action === "UPD-VFC") {
					...
					this.getView().byId("idUnitCost").setEditable(true);
					this.getView().byId("idValueAdded").setEditable(true);
					this.getView().byId("idTgtProfit").setEditable(true);
					...
					this.getView().byId("saveReq").setVisible(false);
					this.getView().byId("sendToApprv").setVisible(false);
					this.getView().byId("idUnitCost").setEditable(false);
					this.getView().byId("idValueAdded").setEditable(false);
					this.getView().byId("idTgtProfit").setEditable(false);
